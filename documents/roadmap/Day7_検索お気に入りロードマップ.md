...
#### 【設計メモ】検索フォームUIワイヤーフレーム・APIパラメータ整理

- **検索フォームUIワイヤーフレーム（テキスト例）**

```
+---------------------------------------------------+
| [店舗名: _________ ] [エリア ▼] [ジャンル ▼] [検索] [クリア]
+---------------------------------------------------+
```

- **UI要素とAPIパラメータのマッピング**

| UI項目   | 入力形式   | APIパラメータ名 | 例値         |
|----------|------------|----------------|--------------|
| 店舗名   | テキスト   | name           | "トキエイツ" |
| エリア   | セレクト   | area           | "渋谷"       |
| ジャンル | セレクト   | genre          | "イタリアン" |

- **APIリクエスト例**

```
GET /api/restaurants?name=トキエイツ&area=渋谷&genre=イタリアン
```

- **動作仕様メモ**
    - 検索ボタン押下で上記APIを呼び出し、結果を一覧に反映。
    - クリアボタンで全件表示に戻す。
    - エリア・ジャンルの選択肢は `/api/tags?category=area` `/api/tags?category=genre` で取得。
    - レスポンシブ対応・shadcn/Tailwindでスタイリング。
